    ...
    <xsl:template match="//div[@class='ps-shoe mb-30'][1]">
        <xsl:element name="product">
            <xsl:element name="name">
                <xsl:value-of select="//a[@class='ps-shoe__name'][1]/text()[1]"></xsl:value-of>
            </xsl:element>
            <xsl:element name="price">
                <xsl:variable name="price" select="//span[@class='ps-shoe__price'][1]/text()[1]"></xsl:variable>
                <xsl:value-of select="translate($price, translate($price, '0123456789', ''), '')"></xsl:value-of>
            </xsl:element>
            <xsl:element name="picture">
                <xsl:variable name="picture" select="//img[@class='lazy'][1]/@src"></xsl:variable>
